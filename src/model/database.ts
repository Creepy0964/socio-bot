...
export function getUserById(userId: number) {
    ...
    return new BotUser(user.userId, user.tId, user.username, user.isBanned);
}
...
export class BotUser implements UserI {
    ...
    setBanned(status: boolean) {
        const user = db.prepare(`UPDATE users SET isBanned = ? WHERE userId = ?`).run(status, this.userId);
